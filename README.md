...
<!-- 📷 Coding View 三栏布局全貌 -->
<p align="center">
  <img src="images/screenshots/coding-view-layout.png" alt="Coding View 布局" width="960" />
</p>
...
<!-- 📷 Diff Review 面板 -->
<p align="center">
  <img src="images/screenshots/coding-diff-review.png" alt="Diff Review" width="960" />
</p>
<p align="center"><sub>Diff Review — 逐块 Accept/Reject · hunk 级精确控制 · 进度可视化</sub></p>
...
<!-- 📷 H5 移动端 Coding View -->
<p align="center">
  <img src="images/screenshots/coding-mobile.png" alt="移动端 Coding View" width="480" />
</p>
...
<!-- 📷 工作流 -->
<img src="images/screenshots/14-workflow.png" alt="工作流" width="300" />
...
<details>
<summary><b>📐 功能全景架构图 — GPT Image 生成提示词</b></summary>

请用以下提示词让 GPT Image 生成 `images/screenshots/architecture-panorama.png`：

```
设计一张现代科技风格的产品功能架构全景图，用于 GitHub README，宽 1920px 高 1080px，深色背景（#0f172a 到 #1e293b 渐变），白色/浅色文字。

产品名：灵犀 AI Agent（本地优先的桌面 AI Agent 操作系统）

布局：分为三大水平层级，从上到下：

【第一层 - 桌面壳 & UI（最上方）】
一个大的半透明圆角卡片，标题 "🖥️ Electron 桌面壳"，内含 4 个小模块：
- "React 19 · 6 套主题 · 流式 UI" 
- "Coding View · Claude Agent SDK"
- "Spotlight 浮窗 · 剪贴板监控"
- "Screen Agent 桌面操控"

【第二层 - Go 后端核心引擎（中间，最大区域）】
一个大的半透明卡片，标题 "⚙️ Go 后端 · 本地 SQLite"，内含 7 个模块卡片 (2行排列)：
- "对话引擎 · 规划 · 工具链"
- "智能体工厂 · 人格蒸馏"
- "自我进化引擎 · Dream 巩固"
- "向量索引 · 混合检索 · 文件监控"
...
【第三层 - AI 运行时（最下方）】
一个半透明卡片，标题 "🧠 AI 运行时"，内含 3 个模块：
- "Claude Agent SDK"
- "Go 协议转换代理（14+ 供应商）"
- "whisper.cpp 离线语音"

层级之间用发光的连接线/箭头表示数据流向（从上到下：UI → 后端 → AI 运行时）。

配色要求：
- 各模块卡片用不同的半透明彩色边框区分（蓝、紫、绿、橙、粉）
- 连接线用渐变发光效果（类似 Vercel/Stripe 那种科技感）
- 整体风格类似 Vercel 或 Linear 的技术架构图
- 左上角放一个小的灵犀 logo 占位圆形（白色描边）
- 右上角标注 "v1.0.184"

字体要求：标题用粗体无衬线字体，模块名用中等粗细，整体清晰可读。
不要使用真实的代码或 UI 截图，纯粹是信息图/架构图风格。
```
