...
<td>纠正、负反馈、有价值长对话自动提炼为长期记忆 / 知识文档 / 技能修复。全局扫描器 + 会话级触发；进化历程全程可审计、可搜索、单条可撤销。记忆巩固 Dream 定时整理/合并/精炼/清理记忆库。</td>
</tr>
<tr>
...
<tr>
<td align="center"><strong>💻 编程视图</strong></td>
<td>Coding Agent 独立模式（与灵犀主模式并肩），启动时可选。完全独立于主界面：独立布局壳、独立设置页、独立暖色调主题。多tab会话栏 + 左侧图标栏 + 会话侧边栏 + 文件树侧栏（搜索过滤+拖拽文件/文件夹引用）+ Workspace Changes 面板 + 底部git状态栏。文件/文件夹chip附件、内联模型/智能体切换、工具聚合卡片颜色编码、diff视图、交互式选择/输入块（AI提问自动渲染为可点击选项卡片，支持裸JSON和代码围栏两种格式）、TaskTodoList吸顶实时进度栏（AI自动生成任务计划并逐步打钩完成）、Agent Team协作面板。代码预览编辑面板（点击文件在聊天上方打开，支持语法高亮+行号+Edit/Save模式+Cmd+S保存）。工作目录上下文自动注入（AI 在正确项目目录工作）。Agent 状态实时追踪（Thinking/Reading/Executing + 计时）。Cmd+K全文搜索。主界面顶栏"Coding"一键切换。H5 移动端响应式。</td>
</tr>
<tr>
...
**记忆巩固 (Dream)**：后台定时巡检记忆库，利用 LLM 自动合并重复记忆、精炼模糊表述、补充近期对话中的新知识、清理过时条目。支持手动触发，实时进度推送，巩固历史可查。
