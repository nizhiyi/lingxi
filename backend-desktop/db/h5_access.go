...
func ValidateH5Token(token string) (*H5AccessToken, error) {
	...
	err := DB.QueryRow(`SELECT id, token_hash, token_preview, label, enabled, expires_at, last_used_at, created_at
		FROM h5_access_tokens WHERE token_hash=?`, hash).
		Scan(&t.ID, &t.TokenHash, &t.TokenPreview, &t.Label, &t.Enabled, &expiresAt, &lastUsedAt, &t.CreatedAt)
	if err != nil {
		return nil, err
	}
	if !t.Enabled {
	...
	// 更新最后使用时间
	DB.Exec(`UPDATE h5_access_tokens SET last_used_at=? WHERE id=?`, time.Now().UTC().Format("2006-01-02 15:04:05"), t.ID)
	return &t, nil
}
