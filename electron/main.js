...
function startBackend() {
  ...
  // 补全 PATH：从用户登录 shell 获取完整 PATH（包含 .zshrc/.bashrc 中配置的路径）
  const userHome = require('os').homedir();
  const isWin = process.platform === 'win32';
  const pathSep = isWin ? ';' : ':';
  let shellPath = '';
  let shellEnvFile = '';
  let extraPaths_lingxiBin = '';
  if (!isWin) {
    const userShell = process.env.SHELL || '/bin/zsh';
    try {
      shellPath = execSync(`${userShell} -l -c "echo \\$PATH"`, {
        timeout: 5000, encoding: 'utf-8',
        env: { ...process.env, HOME: userHome },
      }).trim();
    } catch (e) {
      console.log('[electron] failed to get shell PATH, using fallback:', e.message);
    }
    // ── 方案 A: BASH_ENV（让 bash 非交互模式也能加载 alias/function） ──
    // ── 方案 B: alias → 可执行 wrapper 脚本（兜底，不依赖 BASH_ENV 被支持） ──
    // 两套方案同时生效，确保不管 Claude CLI 用什么 shell 都能找到用户的自定义命令
    try {
      let aliasOutput = '';
      let functionOutput = '';
      try {
        aliasOutput = execSync(`${userShell} -l -i -c "alias" 2>/dev/null`, {
          timeout: 5000, encoding: 'utf-8',
          env: { ...process.env, HOME: userHome },
        }).trim();
      } catch (_) { /* ignore */ }
      try {
        functionOutput = execSync(`${userShell} -l -i -c "typeset -f" 2>/dev/null`, {
          timeout: 5000, encoding: 'utf-8',
          env: { ...process.env, HOME: userHome },
        }).trim();
      } catch (_) { /* ignore */ }

      // 方案 B: 将 alias 转换为可执行 wrapper 脚本，放到灵犀专属 bin 目录
      // 这样 Claude CLI 无论用 bash -c 还是 sh -c 都能直接找到命令
      const lingxiBinDir = path.join(app.getPath('userData'), 'shell-bin');
      try { fs.mkdirSync(lingxiBinDir, { recursive: true }); } catch (_) {}
      // 清理旧的 wrapper 脚本
      try {
        for (const f of fs.readdirSync(lingxiBinDir)) {
          fs.unlinkSync(path.join(lingxiBinDir, f));
        }
      } catch (_) {}
      ...
      if (aliasOutput) {
        for (const line of aliasOutput.split('\n')) {
          // 解析 alias：zsh 格式 "name='value'" 或 bash 格式 "alias name='value'"
          let name, target;
          const bashMatch = line.match(/^alias\s+([\w.-]+)=['"](.*?)['"]$/);
          const zshMatch = line.match(/^([\w.-]+)=['"](.*?)['"]$/);
          if (bashMatch) { name = bashMatch[1]; target = bashMatch[2]; }
          else if (zshMatch) { name = zshMatch[1]; target = zshMatch[2]; }
          if (!name || !target) continue;
          // 跳过系统内置（ls/grep 等通常只是加了颜色参数）
          if (['ls', 'grep', 'll', 'la', 'l', 'run-help', 'which-command'].includes(name)) continue;
          const wrapperPath = path.join(lingxiBinDir, name);
          // wrapper 脚本：透传所有参数给原始命令
          const wrapperContent = `#!/bin/bash\nexec ${target} "$@"\n`;
          fs.writeFileSync(wrapperPath, wrapperContent, { mode: 0o755 });
          wrapperCount++;
        }
      }
      console.log(`[electron] created ${wrapperCount} command wrappers in ${lingxiBinDir}`);

      // 方案 A: 生成 BASH_ENV 文件（作为补充，支持更复杂的 alias/function）
      if (aliasOutput || functionOutput) {
        shellEnvFile = path.join(app.getPath('userData'), '.lingxi-shell-env.sh');
        const userBinPaths = [
          lingxiBinDir,
          `${userHome}/.local/bin`,
          `${userHome}/bin`,
        ...
        const envLines = [
          '#!/bin/bash',
          '# Auto-generated by 灵犀 — user shell env for non-interactive bash',
          'shopt -s expand_aliases 2>/dev/null',
          `export PATH="${fullPath}"`,
          '',
        ];
        if (aliasOutput) {
          envLines.push('# --- aliases ---');
          for (const line of aliasOutput.split('\n')) {
            if (line.startsWith('alias ')) { envLines.push(line); continue; }
            const eq = line.indexOf('=');
            if (eq > 0) envLines.push(`alias ${line}`);
          }
          envLines.push('');
        }
        if (functionOutput) {
          envLines.push('# --- functions ---');
          envLines.push(functionOutput);
          envLines.push('');
        }
        fs.writeFileSync(shellEnvFile, envLines.join('\n'), { mode: 0o755 });
        const aliasCount = aliasOutput ? aliasOutput.split('\n').length : 0;
        const funcCount = functionOutput ? (functionOutput.match(/^(\w+)\s*\(\)/gm) || []).length : 0;
        console.log(`[electron] generated BASH_ENV file: ${shellEnvFile} (${aliasCount} aliases, ${funcCount} functions)`);
      }

      // 确保 lingxiBinDir 在 extraPaths 中（后面合并到 PATH）
      extraPaths_lingxiBin = lingxiBinDir;
    } catch (e) {
      console.log('[electron] failed to extract shell env:', e.message);
    }
  }
  ...
  const allPaths = [
    ...(extraPaths_lingxiBin ? [extraPaths_lingxiBin] : []),
    ...extraPaths,
    ...(shellPath ? shellPath.split(':') : []),
    ...currentPath.split(pathSep),
  ];
  const mergedPath = [...new Set(allPaths)].join(pathSep);
  ...
  const envObj = {
    ...
    USER_HOME: userHome,
    ...(shellEnvFile ? { LINGXI_SHELL_ENV: shellEnvFile } : {}),
    ...getSDKRunnerEnv(),
    ...authEnv,
  };
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const platformHints = isMacPlatform
      ? `• macOS 安全策略阻止了应用运行\n` +
        `• 请尝试右键 → 打开 或执行: xattr -cr "/Applications/灵犀.app"\n`
      : `• Windows 防火墙或杀毒软件阻止了后端进程\n` +
        `• 请尝试以管理员身份运行，或将灵犀加入杀毒软件白名单\n`;
