...
# ── 自动递增版本号（patch +1）──────────────────────────────────
PKG_JSON="./electron/package.json"

# 兼容 Windows Git Bash
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
  # ✅ 强制转成正斜杠（Node 最喜欢）
  PKG_JSON_WIN=$(cygpath -u "$PKG_JSON")
  CURRENT_VERSION=$(node -e "console.log(require('$PKG_JSON_WIN').version)")
else
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
  ...
  node -e "
    const fs = require('fs');
    const p = JSON.parse(fs.readFileSync('$PKG_JSON_WIN', 'utf8'));
    p.version = '$NEW_VERSION';
    fs.writeFileSync('$PKG_JSON_WIN', JSON.stringify(p, null, 2) + '\n');
  "
else
  node -e "
    const fs = require('fs');
